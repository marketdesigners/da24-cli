...
export function getApiKey() {
  ...
  // NOTE: Cannot distinguish between "no config" vs "config exists but no key".
  // This is acceptable since inquiry.js only checks if key exists (truthy check).
  return null;
}
...
export function saveMcpConfig(apiKey) {
  // apiKey이 null이면 견적 계산 전용 URL (유효), 빈 문자열은 거부
  if (apiKey !== null && apiKey !== undefined && apiKey.trim() === '') {
    throw new Error('apiKey는 비어있는 문자열을 허용하지 않습니다. null을 사용하세요.');
  }
