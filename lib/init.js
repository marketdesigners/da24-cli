...
const PLUGIN_DIR = join(homedir(), '.claude', 'plugins', 'marketplaces', 'da24');
const SETTINGS_PATH = join(homedir(), '.claude', 'settings.json');

function installPlugin() {
  // commands 디렉토리에 da24.md 복사
  const commandsDir = join(PLUGIN_DIR, 'commands');
  mkdirSync(commandsDir, { recursive: true });
  copyFileSync(join(__dirname, '..', 'commands', 'da24.md'), join(commandsDir, 'da24.md'));
  ...
  settings.enabledPlugins['da24@da24'] = true;

  writeFileSync(SETTINGS_PATH, JSON.stringify(settings, null, 4), 'utf-8');
}
